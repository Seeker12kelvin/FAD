...
const HeroSection4 = () => {
  ...
  return (
    <section className="flex flex-col gap-10 p-10">
      ...
      <div className="flex gap-6 min-h-211.5 h-full">
        <div className="max-w-225.5 w-full grid grid-cols-2 grid-rows-2 gap-10 ">
          {list.map((data) => (
            <div
              key={data.id}
              className="max-w-109.5 w-fit min-h-92.25 h-fit flex flex-col gap-4"
            >
              <img
                src={data.img}
                alt=""
                className="w-full h-66 object-cover object-top"
              />
        ...
        <div className="flex-1 bg-[#667085] max-w-[0.5px]" />

        <img
          src={fieldNoteSectionPic5}
          alt=""
          className="object-cover flex-1 max-w-96.5"
        />
      </div>
    </section>
  );
